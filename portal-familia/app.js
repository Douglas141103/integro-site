...
async function loadStudentsForCurrentProfile(profile) {
  ...
  await loadSchoolsByIds([...new Set((students || []).map(item => item.school_id).filter(Boolean))]);
...
async function loadAttendanceSchedule(studentId) {
  const { data, error } = await supabase
...
async function loadAttendanceRecords(studentId) {
  const { start, end } = monthBounds(state.attendanceMonth);

  const { data, error } = await supabase
    .from('student_attendance')
    .select('id, attendance_date, status, notes, created_at')
    .eq('student_id', studentId)
...
async function loadMessages(studentId, profileId) {
  ...
  const ids = [
    ...new Set(
      messages
        .flatMap(item => [item.sender_profile_id, item.recipient_profile_id])
        .filter(Boolean)
    )
  ];
...
function cleanPlanText(description = '') {
  let text = String(description || '').trim();

  text = text
    .replace(/PLANO DE ESTUDO DA SEMANA\s*—\s*INTEGRO/gi, '')
    .replace(/Aluno:\s*[^\n]+/gi, '')
    .replace(/Área:\s*[^\n]+/gi, '')
    .replace(/Semana:\s*[^\n]+/gi, '')
    .replace(/\s+/g, ' ')
...
function extractPlanSections(description = '') {
  let text = String(description || '').trim();
  ...
  const labels = [
    {
      raw: 'O QUE SERÁ TRABALHADO:',
      title: 'O que será trabalhado'
    },
    {
      raw: 'COMO SERÁ FEITO:',
      title: 'Como será feito'
    },
    {
      raw: 'OBJETIVO DA SEMANA:',
      title: 'Objetivo da semana'
    },
    {
      raw: 'ORIENTAÇÃO PARA A FAMÍLIA:',
  ...
  labels.forEach(label => {
    const escaped = label.raw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  ...
  const firstIndex = labels
    .map(label => upper.indexOf(label.raw))
    .filter(index => index >= 0)
    .sort((a, b) => a - b)[0];

  if (typeof firstIndex === 'number' && firstIndex > 0) {
    text = text.slice(firstIndex).trim();
  }

  const sections = [];

  labels.forEach((label, index) => {
    const currentIndex = text.toUpperCase().indexOf(label.raw);

    if (currentIndex === -1) {
      return;
    }

    const start = currentIndex + label.raw.length;

    const nextIndexes = labels
      .slice(index + 1)
      .map(nextLabel => text.toUpperCase().indexOf(nextLabel.raw))
      .filter(nextIndex => nextIndex > currentIndex)
      .sort((a, b) => a - b);

    const end = nextIndexes.length ? nextIndexes[0] : text.length;
...
function renderPlanDescription(description = '') {
  const sections = extractPlanSections(description);

  if (!sections.length) {
    return `
      <div class="family-plan-text">
        ${formatPlanText(description || 'Sem descrição informada.')}
      </div>
    `;
  }

  return `
    <div class="family-plan-sections">
      ${sections.map(section => `
        <section class="family-plan-section">
          <h5>${escapeHtml(section.title)}</h5>
          <p>${formatPlanText(section.content)}</p>
        </section>
      `).join('')}
    </div>
  `;
...
function renderPlans(items) {
  ...
  list.innerHTML = items.length
    ? items.map(item => `
      <article class="item-card family-plan-card">
        <div class="badge">${escapeHtml(item.week_reference || 'Planejamento')}</div>

        <h4>${escapeHtml(item.title || 'Plano de estudo da semana')}</h4>

        <div class="item-meta">
          Status: ${escapeHtml(item.status || 'ativo')} • Atualizado em ${formatDate(item.created_at)}
        </div>

        ${renderPlanDescription(item.description)}
      </article>
    `).join('')
    : '<div class="empty-state">Ainda não há um plano de estudos lançado para este aluno.</div>';
}
  ...
  list.innerHTML = items.length
    ? items.map(item => `
      <article class="item-card">
        ...
        <button class="btn btn-primary open-material" type="button" data-file="${escapeHtml(item.file_path)}">
          Abrir arquivo
        </button>
      </article>
    `).join('')
    : '<div class="empty-state">Ainda não há materiais individualizados para este aluno.</div>';
...
function renderProgress(items) {
  ...
  list.innerHTML = items.length
    ...
    `).join('')
    : '<div class="empty-state">Ainda não há registros de evolução para este aluno.</div>';
}
...
function renderAnnouncements(items) {
  ...
  list.innerHTML = items.length
    ...
    `).join('')
    : '<div class="empty-state">Nenhum comunicado disponível no momento.</div>';
}

function renderAttendance(schedule = [], records = []) {
  ...
  const monthInput = document.getElementById('family-attendance-month');

  if (count) count.textContent = String(records.length);
  if (monthInput) monthInput.value = monthValue(state.attendanceMonth);

  const scheduledWeekdays = new Set(schedule.map(item => Number(item.weekday)));

  if (scheduleEl) {
    scheduleEl.innerHTML = scheduledWeekdays.size
      ? `<strong>Dias de atendimento:</strong> ${[...scheduledWeekdays].sort((a, b) => a - b).map(day => weekdayNames[day]).join(', ')}`
      : '<div class="empty-state">Ainda não há dias fixos de atendimento cadastrados.</div>';
  }

  if (calendarEl) {
    const recordMap = new Map(records.map(item => [item.attendance_date, item]));
    const first = new Date(state.attendanceMonth.getFullYear(), state.attendanceMonth.getMonth(), 1);
    const last = new Date(state.attendanceMonth.getFullYear(), state.attendanceMonth.getMonth() + 1, 0);
    const cells = [];
    ...
    for (let day = 1; day <= last.getDate(); day++) {
      const current = new Date(state.attendanceMonth.getFullYear(), state.attendanceMonth.getMonth(), day);
      const iso = formatDateOnly(current);
      const record = recordMap.get(iso);
      const scheduled = scheduledWeekdays.has(current.getDay());
      const status = record?.status || '';
      const label = record ? attendanceLabel(record.status) : (scheduled ? 'dia previsto' : '—');

      cells.push(`
        <div class="calendar-day ${scheduled ? 'scheduled' : ''} ${status ? `status-${status}` : ''}">
          <strong>${day}</strong>
  ...
  if (listEl) {
    listEl.innerHTML = records.length
      ? records.map(item => `
        <article class="item-card">
          <div class="badge">${escapeHtml(attendanceLabel(item.status))}</div>
          <h4>${formatDateBR(item.attendance_date)}</h4>
          <p>${escapeHtml(item.notes || 'Sem observação.')}</p>
        </article>
      `).join('')
      : '<div class="empty-state">Ainda não há frequência lançada para este mês.</div>';
  }
}
...
function renderMessages(items, currentProfileId) {
  ...
  list.innerHTML = items.length
    ...
    : '<div class="empty-state">Ainda não há recados trocados com a equipe.</div>';
}
...
async function refreshStudentView() {
  ...
  const [
    ...
    loadAttendanceSchedule(state.selectedStudent.id),
    loadAttendanceRecords(state.selectedStudent.id)
  ]);
  ...
  renderAttendance(attendanceSchedule, attendanceRecords);
}
...
async function sendMessage(event) {
  ...
  try {
    ...
    const payload = {
      ...
      recipient_profile_id: state.teacherRecipientId || null,
      message: text
    };
    ...
    renderMessages(await loadMessages(state.selectedStudent.id, state.profile.id), state.profile.id);

    setFeedback(
      state.teacherRecipientId
        ? 'Recado enviado com sucesso.'
        : 'Recado salvo, mas ainda não há professor vinculado a este aluno.',
      'success'
    );
  } catch (error) {
    console.error(error);
...
async function handleDashboardPage() {
  ...
  document.getElementById('family-attendance-month')?.addEventListener('change', async (event) => {

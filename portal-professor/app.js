...
function setupActionNavigation() {
  ...
  triggers.forEach(trigger => {
    trigger.addEventListener('click', () => showSection(trigger.dataset.sectionTarget || 'actionHome'));
  });

  backButtons.forEach(button => {
    button.addEventListener('click', () => showSection('actionHome'));
  });
...
async function initDashboardPage() {
  ...
  document.addEventListener('click', async (event) => {
    ...
    if (!materialId) {
      setHelperMessage('materialMessage', 'Não foi possível identificar o material para exclusão.', 'error');
      return;
    }

    const confirmed = window.confirm('Tem certeza que deseja excluir este material do aluno?');
    if (!confirmed) return;
    ...
    try {
      ...
      setHelperMessage('materialMessage', error.message || 'Erro ao excluir material.', 'error');

(() => {
  ...
  function moduleName(data, id) { return id ? data.modules.find((m) => m.id === id)?.name || "Módulo não localizado" : "Sem módulo/matéria"; }
  function rowsModules(data) { return data.modules.length ? data.modules.map((m) => `<tr><td>${safe(m.order_index || "—")}</td><td>${safe(m.name)}</td><td>${safe(m.workload_hours || "—")}</td><td>${safe(m.description || "—")}</td></tr>`).join("") : `<tr><td colspan="4">Nenhum módulo/matéria cadastrado.</td></tr>`; }
  function rowsGrades(data) { return data.assessments.length ? data.assessments.map((a) => { const g = data.grades.find((x) => x.assessment_id === a.id); return `<tr><td>${safe(formatDateBR(a.assessment_date))}</td><td>${safe(moduleName(data, a.course_module_id))}</td><td>${safe(a.title)}</td><td>${safe(a.assessment_type)}</td><td>${safe(g?.score ?? "—")}</td><td>${safe(a.max_score || 10)}</td></tr>`; }).join("") : `<tr><td colspan="6">Nenhuma avaliação registrada.</td></tr>`; }
  function openPrint(html) { const win = window.open("", "_blank"); if (!win) return alert("Permita pop-ups para imprimir."); win.document.open(); win.document.write(html); win.document.close(); setTimeout(() => win.print(), 350); }
  async function updateSummary(data) { await client.from("course_enrollments").update({ final_average: data.average, attendance_percentage: data.frequency, final_result: data.result, updated_at: new Date().toISOString() }).eq("id", data.enrollment.id).eq("school_id", data.ctx.school.id); }

  async function printReport(event) {
    event.preventDefault(); event.stopImmediatePropagation();
    try {
      const d = await loadData(); await updateSummary(d);
      ...
      openPrint(html); show("Boletim gerado com módulos/matérias.", "ok");
    } catch (error) { console.error(error); show(error.message || "Erro ao gerar boletim.", "error"); }
  }
  ...
  async function printCertificate(event) {
    ...
    try {
      ...
      openPrint(html); show("Certificado emitido com módulos/matérias.", "ok");
    } catch (error) { console.error(error); show(error.message || "Erro ao emitir certificado.", "error"); }
  }

  document.addEventListener("DOMContentLoaded", () => { $("printReportBtn")?.addEventListener("click", printReport, true); $("printCertificateBtn")?.addEventListener("click", printCertificate, true); });
})();

...
async function loadCompanySettings() {
  ...
  if (error) {
    console.warn('Não foi possível carregar dados da empresa:', error.message);
    state.companySettings = null;
    return;
  }
...
function getCompanyDocumentLine() {
  const company = state.companySettings;
  ...
  return `${company.document_type || 'CNPJ'}: ${company.document_number}`;
}
...
function getCompanyAddressLine() {
  ...
  const zip = company.address_zip_code
...
function getCompanyContactLine() {
  const company = state.companySettings;
...
function getReceiptFooterText() {
  ...
  return (
    ...
    'Este recibo comprova o pagamento informado e foi emitido pelo sistema de gestão do INTEGRO.'
  );
}

function buildCompanyHeaderHtml() {
  const companyName = getCompanyName();
  const companyDocumentLine = getCompanyDocumentLine();
  const companyAddressLine = getCompanyAddressLine();
  const companyContactLine = getCompanyContactLine();

  return `
    <div class="company-header">
      <h2>${escapeHtml(companyName)}</h2>
      ${companyDocumentLine ? `<p>${escapeHtml(companyDocumentLine)}</p>` : ''}
      ${companyAddressLine ? `<p>${escapeHtml(companyAddressLine)}</p>` : ''}
      ${companyContactLine ? `<p>${escapeHtml(companyContactLine)}</p>` : ''}
    </div>
  `;
}
...
async function loadStudents() {
  const { data, error } = await client
    .from('students')
    .select('id, full_name, active, guardian_1_name, guardian_1_cpf, guardian_1_email, guardian_1_phone')
    .eq('school_id', state.school.id)
    .order('full_name');
...
function renderPackages() {
  $('packagesList').innerHTML = state.packages.length
    ...
    `).join('')
    : '<p class="muted">Nenhum pacote cadastrado ainda.</p>';
}

function renderDiscounts() {
  $('discountsList').innerHTML = state.discounts.length
    ? state.discounts.map((d) => {
      const value = d.discount_type === 'percentual'
        ? `${d.value}%`
        : money(d.value);
        ...
        <article class="record">
          <div class="record-head">
            <div>
              ...
              <small>${d.discount_type === 'percentual' ? 'Percentual' : 'Valor fixo'} • ${d.active ? 'Ativo' : 'Inativo'}</small>
            </div>
    ...
    : '<p class="muted">Nenhum desconto cadastrado ainda.</p>';
}

function renderEntries() {
  $('entriesList').innerHTML = state.entries.length
    ? state.entries.map((e) => `
      <article class="record">
        <div class="record-head">
          <div>
        ...
        <p>
          <span class="badge">${escapeHtml(entryTypeLabel(e.entry_type || 'mensalidade'))}</span>
        </p>
        ...
        <button class="linkbtn" type="button" onclick='printExistingReceipt(${JSON.stringify(e).replace(/'/g, "&#039;")})'>
          Imprimir recibo
        </button>
      </article>
    `).join('')
    : '<p class="muted">Nenhuma entrada registrada ainda.</p>';
}
  ...
  $('expensesList').innerHTML = state.expenses.length
    ? state.expenses.map((e) => `
      <article class="record">
        <div class="record-head">
          <div>
            <strong>${escapeHtml(e.description)}</strong><br>
            <small>${new Date(e.expense_date + 'T00:00:00').toLocaleDateString('pt-BR')} • ${escapeHtml(e.category || 'Sem categoria')}</small>
          </div>
    ...
    `).join('')
    : '<p class="muted">Nenhuma saída registrada ainda.</p>';
}
...
function applyDiscount() {
  ...
  if (d) {
    discount = d.discount_type === 'percentual'
      ? gross * (Number(d.value || 0) / 100)
      : Number(d.value || 0);
  }
...
function printReceipt(entry) {
  ...
  const companyName = getCompanyName();
  const companyLegalName = getCompanyLegalName();
  const companyDocumentLine = getCompanyDocumentLine();
  const companyAddressLine = getCompanyAddressLine();
  const companyContactLine = getCompanyContactLine();
  const receiptFooterText = getReceiptFooterText();

  const typeLabel = entryTypeLabel(entry.entry_type || 'mensalidade');

  const description =
    entry.description ||
    entry.package_name_snapshot ||
    `Pagamento de ${typeLabel.toLowerCase()}.`;

  const companyHeader = `
    <div class="company-header">
      <h2>${escapeHtml(companyName)}</h2>
      ${companyLegalName && companyLegalName !== companyName ? `<p>${escapeHtml(companyLegalName)}</p>` : ''}
      ${companyDocumentLine ? `<p>${escapeHtml(companyDocumentLine)}</p>` : ''}
      ${companyAddressLine ? `<p>${escapeHtml(companyAddressLine)}</p>` : ''}
      ${companyContactLine ? `<p>${escapeHtml(companyContactLine)}</p>` : ''}
    </div>
  ...
  const via = (label) => `
    <section class="receipt">
      ...
      <div class="header">
        <h1>RECIBO Nº ${receiptNo}</h1>
        <strong>${escapeHtml(label)}</strong>
      </div>

      <p><strong>Emitente:</strong> ${escapeHtml(companyName)}</p>
      ${companyDocumentLine ? `<p><strong>Documento do emitente:</strong> ${escapeHtml(companyDocumentLine)}</p>` : ''}
      ...
      <div class="receipt-footer">
        ${escapeHtml(receiptFooterText)}
      </div>

      <div class="signature">
        <span></span>
        <p>Assinatura / ${escapeHtml(companyName)}</p>
      </div>
    </section>
  `;
  ...
  $('reportPreview').innerHTML = `
    ...
    <div class="report-grid">
      <div class="report-stat">
        ...
        <strong>${money(report.totalEntries)}</strong>
      </div>
      ...
      <div class="report-stat">
        ...
        <strong>${report.entries.length + report.expenses.length}</strong>
      </div>
    </div>
  `;
}
...
async function downloadCsvReport(event) {
  ...
  try {
    ...
    const summary = [
      ...
      ['Documento da empresa', getCompanyDocumentLine()],
      ['Endereço', getCompanyAddressLine()],
      ['Contato', getCompanyContactLine()],
  ...
  try {
    ...
    const trs = rows.map((r) => `
      <tr>
        ...
        <td>${escapeHtml(r.aluno_ou_beneficiario)}</td>
        <td>${escapeHtml(r.pessoa_destino)}</td>
...
<body>
  ...
  <p><strong>Período:</strong> ${escapeHtml(report.label)}</p>

  <div class="summary">
    <div>
      <small>Entradas</small>
      <h2>${money(report.totalEntries)}</h2>
    </div>

    <div>
      <small>Saídas</small>
      <h2>${money(report.totalExpenses)}</h2>
    </div>

    <div>
      <small>Saldo</small>
      <h2>${money(report.balance)}</h2>
    </div>
  </div>
  ...
  <table>
    ...
    <tbody>
      ${trs || '<tr><td colspan="8">Nenhum movimento no período.</td></tr>'}
    </tbody>
  </table>
</body>
</html>
